...
parser = argparse.ArgumentParser(description='PyTorch Two-Stream Action Recognition with TSN')
...
parser.add_argument('--arch', '-a', metavar='ARCH', default='rgb_openpose_resnet152',
                    choices=model_names,
...
parser.add_argument('--segment_number', default=3, type=int, metavar='N',
                    help='Number of segments in temporal segment networks (default: 3)')
...
def main():
    ...
    saveLocation="./checkpoint/"+args.dataset+"_tsn_"+args.arch+"_split"+str(args.split)
    ...
    train_dataset = datasets.__dict__[args.dataset](root=dataset,
                                                    ...
                                                    num_segments=args.segment_number)
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (inputs, targets) in enumerate(train_loader):
        ...
        inputsShape=inputs.shape
        N=inputsShape[0]
        inputsReshaped=inputs.view([N*args.segment_number,-1,inputsShape[2],inputsShape[3]])
        
        output_var=model(inputsReshaped)
        output1 = output_var[:N,:]
        output2 = output_var[N:2*N,:]
        output3 = output_var[2*N:3*N,:]
        ...
        if (i+1) % args.iter_size == 0:
            ...
            losses.update(loss_mini_batch, inputs.size(0))
            writer.add_scalar('data/loss', loss_mini_batch, trainLoaderLength*epoch+i)
            top1.update(acc_mini_batch/args.iter_size, inputs.size(0))
            writer.add_scalar('data/top1', acc_mini_batch/args.iter_size, trainLoaderLength*epoch+i)
