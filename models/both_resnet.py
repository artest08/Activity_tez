...
from .BERT.bert import BERT, BERT2, BERT3, BERT4, BERT5, BERT5_BOTH
import torch.utils.model_zoo as model_zoo
from .flow_resnet import flow_resnet18
from .rgb_resnet import rgb_resnet18

__all__ = ['both_resnet18_bert10','both_resnet18_bert10X']
...
class both_resnet18_bert10X(nn.Module):
    def __init__(self, num_classes , length, modelPath=''):
        super(both_resnet18_bert10X, self).__init__()
        self.hidden_size=512
        self.n_layers=1
        self.attn_heads=8
        self.num_classes=num_classes
        ...
        self.avgpool = nn.AvgPool2d(7)
        self.bert = BERT5_BOTH(512,self.length, hidden=self.hidden_size, n_layers=self.n_layers, attn_heads=self.attn_heads)
        print(sum(p.numel() for p in self.bert.parameters() if p.requires_grad))
        self.fc_action_rgb = nn.Linear(512, num_classes)
        self.fc_action_flow = nn.Linear(512, num_classes)
        ...
        torch.nn.init.xavier_uniform_(self.fc_action_flow.weight)
        self.fc_action_rgb.bias.data.zero_()
    
        torch.nn.init.xavier_uniform_(self.fc_action_flow.weight)
        self.fc_action_flow.bias.data.zero_()
    ...
    def forward(self, x):
        ...
        input_vectors = x_rgb
        output_rgb, output_flow, maskSample = self.bert(x_rgb,x_flow)
        classificationOut_rgb = output_rgb[:,0,:]
        classificationOut_flow = output_flow[:,0,:]
        sequenceOut = output_rgb[:,1:,:]
        output_rgb = self.dp(classificationOut_rgb)
        output_flow = self.dp(classificationOut_flow)
        x_rgb = self.fc_action_rgb(output_rgb)
        x_flow = self.fc_action_flow(output_flow)
        return x_rgb, x_flow, input_vectors, sequenceOut, maskSample