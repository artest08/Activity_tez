...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 28 16:53:08 2020
...
from utils.architecture_transform import determine_architecture_transform
...
parser.add_argument('--arch', '-a', metavar='ARCH', default='rgb_MFNET3D64f_16x4_ensemble2_112',
                    choices=model_names,
                    help='model architecture: ' +
                        ' | '.join(model_names))
parser.add_argument('--arch_teacher1', '-teacher1', metavar='ARCH', default='rgb_r2plus1d_32f_34',
                    choices=model_names,
                    help='model architecture: ' +
                        ' | '.join(model_names))
parser.add_argument('--arch_teacher2', '-teacher2', metavar='ARCH', default='rgb_slowfast64f_50',
                    choices=model_names,
                    help='model architecture: ' +
                        ' | '.join(model_names))
parser.add_argument('--arch_teacher3', '-teacher3', metavar='ARCH', default='rgb_resneXt3D64f101_student_mars',
                    choices=model_names,
                    help='model architecture: ' +
                        ' | '.join(model_names))
...
def main():
    global args, best_prec1,model ,writer, best_loss, length, width, height, model_teacher1, model_teacher2, model_teacher3
    ...
    if args.evaluate:
        ...
        model_path = os.path.join(modelLocation1,'model_best.pth.tar') 
        ...
        model_path = os.path.join(modelLocation2,'model_best.pth.tar') 
        params = torch.load(model_path)
        model_teacher2.load_state_dict(params['state_dict'])
        for param in model_teacher2.parameters():
            param.requires_grad = False
            
        model_teacher3 = build_model(args.arch_teacher3)
        modelLocation3="./checkpoint/"+args.dataset+"_"+args.arch_teacher3+"_split"+str(args.split)

        model_path = os.path.join(modelLocation3,'model_best.pth.tar') 
        params = torch.load(model_path)
        model_teacher3.load_state_dict(params['state_dict'])
        for param in model_teacher3.parameters():
            param.requires_grad = False
    ...
    architecture_name_list= [args.arch, args.arch_teacher1, args.arch_teacher2, args.arch_teacher3]


    transform_list = determine_architecture_transform(architecture_name_list, args.num_seg, length)
...
def train(train_loader, model, criterion, criterion_mse, optimizer, epoch):
    ...
    mse_loss_coeff = torch.tensor(50 / 3).cuda()
    ...
    for i, (input_student, input_teacher1, input_teacher2, input_teacher3, targets) in enumerate(train_loader):
        input_student = input_student.view(-1, length, 3, input_student.shape[-1], input_student.shape[-1]).transpose(1,2)
        input_teacher1 = input_teacher1.view(-1,length,3, input_teacher1.shape[-1], input_teacher1.shape[-1]).transpose(1,2)
        input_teacher2 = input_teacher2.view(-1,length,3, input_teacher2.shape[-1], input_teacher2.shape[-1]).transpose(1,2)
        input_teacher3 = input_teacher3.view(-1,length,3, input_teacher3.shape[-1], input_teacher3.shape[-1]).transpose(1,2)
       
        input_student = input_student.cuda()
        input_teacher1 = input_teacher1.cuda()
        input_teacher2 = input_teacher2.cuda()
        input_teacher3 = input_teacher3.cuda()
        
        targets = targets.cuda()
        
        output, features_student1, features_student2, features_student3 = model.ensemble_forward(input_student)
        features_teacher1 = model_teacher1.mars_forward(input_teacher1)
        features_teacher2 = model_teacher2.mars_forward(input_teacher2)
        features_teacher3 = model_teacher3.mars_forward(input_teacher3)
        ...
        lossMSE1 = criterion_mse(features_student1, features_teacher1)
        lossMSE2 = criterion_mse(features_student2, features_teacher2)
        lossMSE3 = criterion_mse(features_student3, features_teacher3)
        
        lossMSE = lossMSE1 + lossMSE2 + lossMSE3 
...
def validate(val_loader, model, criterion, criterion_mse):
    ...
    with torch.no_grad():
        for i,  (input_student, input_teacher1, input_teacher2, input_teacher3, targets) in enumerate(val_loader):
            input_student = input_student.view(-1, length, 3, input_student.shape[-1], input_student.shape[-1]).transpose(1,2)
            input_teacher1 = input_teacher1.view(-1,length,3, input_teacher1.shape[-1], input_teacher1.shape[-1]).transpose(1,2)
            input_teacher2 = input_teacher2.view(-1,length,3, input_teacher2.shape[-1], input_teacher2.shape[-1]).transpose(1,2)
            input_teacher3 = input_teacher3.view(-1,length,3, input_teacher3.shape[-1], input_teacher3.shape[-1]).transpose(1,2)
