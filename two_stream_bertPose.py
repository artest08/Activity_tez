...
parser.add_argument('--arch', '-a', metavar='ARCH', default='rgb_poseNet_bert2',
                    choices=model_names,
...
def main():
    global args, best_prec1,model,writer,best_loss,mseLossScaleParam, warmUpEpoch
...
#                                weight_decay=args.weight_decay)
    ...
    if modality == "rgb":
        is_color = True
        scale_ratios = [1.0, 0.95, 0.9, 0.85]
        clip_mean = [0.485, 0.456, 0.406] * args.num_seg
        clip_std = [0.229, 0.224, 0.225] * args.num_seg
    elif modality == "flow":
        is_color = False
        scale_ratios = [1.0, 0.875, 0.75]
        clip_mean = [0.5, 0.5] * args.num_seg * args.new_length
        clip_std = [0.226, 0.226] * args.num_seg * args.new_length
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.save_freq == 0:
            prec1,prec3,loss1 = validate(val_loader, model, criterion,criterion2)
            writer.add_scalar('data/top1_validation', prec1, epoch)
            writer.add_scalar('data/top3_validation', prec3, epoch)
            writer.add_scalar('data/classification_loss_validation', loss1, epoch)
...
def build_model():
    if args.dataset=='ucf101':
        model = models.__dict__[args.arch](num_classes=101,length=args.num_seg)
    elif args.dataset=='hmdb51':
        model = models.__dict__[args.arch](num_classes=51, length=args.num_seg)  
    model = model.cuda()
...
def train(train_loader, model, criterion, criterion2, optimizer, epoch):
    ...
    optimizer.zero_grad()
    loss_mini_batch1 = 0.0
    loss_mini_batch2 = 0.0
    acc_mini_batch = 0.0
    ...
    for i, (inputs, targets) in enumerate(train_loader):
        ...
        loss2 = criterion2(input_vectors, sequenceOut)
        
        loss1 = loss1 / args.iter_size
        loss2 = loss2 / args.iter_size
        
        totalLoss=loss1+loss2
        loss_mini_batch1 += loss1.data.item()
        loss_mini_batch2 += loss2.data.item()
        loss1.backward()
        
        totalIterSize+=output.size(0)
        ...
        if (i+1) % args.iter_size == 0:
            ...
            losses1.update(loss_mini_batch1, totalIterSize)
            losses2.update(loss_mini_batch2, totalIterSize)
            top1.update(acc_mini_batch/args.iter_size, totalIterSize)
            top3.update(acc_mini_batch_top3/args.iter_size, totalIterSize)
        ...
        if (i+1) % args.print_freq == 0:
            ...
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Classification Loss {loss1.val:.4f} ({loss1.avg:.4f})\t'
                  'MSE Loss {loss2.val:.4f} ({loss2.avg:.4f})\t'
                  'Prec@1 {top1.val:.3f} ({top1.avg:.3f})'.format(
                   epoch, i+1, len(train_loader)+1, batch_time=batch_time, loss1=losses1,loss2=losses2, top1=top1))
    ...
    print('Epoch: [{0}][{1}/{2}]\t'
          ...
          'Prec@1 {top1.val:.3f} ({top1.avg:.3f})'.format(
           epoch, i+1, len(train_loader)+1, batch_time=batch_time, loss1=losses1,loss2=losses2, top1=top1))            
    writer.add_scalar('data/classification_loss_training', losses1.avg, epoch)
    writer.add_scalar('data/mse_loss_training', losses2.avg, epoch)
    writer.add_scalar('data/total_loss_training', losses2.avg+losses1.avg, epoch)
...
def validate(val_loader, model, criterion,criterion2):
    ...
    with torch.no_grad():
        for i, (inputs, targets) in enumerate(val_loader):
            ...
            # measure accuracy and record loss
            prec1, prec3 = accuracy(output.data, targets, topk=(1, 3))
            losses1.update(loss1.data.item(), output.size(0))
            losses2.update(loss2.data.item(), output.size(0))
        ...
        print(' * Prec@1 {top1.avg:.3f} Prec@3 {top3.avg:.3f} Classification Loss {loss1.avg:.4f} MSE Loss {loss2.avg:.4f}'
              .format(top1=top1, top3=top3, loss1=losses1,loss2=losses2))

    return top1.avg, top3.avg, losses1.avg
